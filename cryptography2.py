...
def print5(chars):
    per = 5
    line = " ".join(chars[i:i+per] for i in range(0, len(chars), per))
    print(line)
    ...
    def encrypt(message, key):
        ...
        return "".join(
            map(
                offset,
                message,
                list(map(lambda x: ALPHABET.index(x), key))
                * (len(message) // len(key) + 1),
            )
        )
    ...
    def decrypt(ciphertext, key):
        ...
        return "".join(
            map(
                offset,
                ciphertext,
                list(map(lambda x: 26 - ALPHABET.index(x), key))
...
class AutoKeyVigenere:
    ...
    def encrypt(message, key):
        return Vigenere.encrypt(
                message, AutoKeyVigenere.lengthenKey(message, key))
    ...
    def encrypt(message, key):
        ...
        for i in range(len(message)):
            ciphertext = ciphertext + chr(
                (ord(message[i]) + ord(key[i % len(key)])) % 256
            )
        return ciphertext
...
class Affine:
    ...
    def encrypt(message, key):
        return "".join(
            ALPHABET[
                (ALPHABET.index(ch) * key[0] + key[1]) % 26
                ] for ch in message
        )
    ...
    def decrypt(ciphertext, key):
        try:
            return "".join(
                ALPHABET[
                    Affine.
                    modReverse(key[0], 26) * (ALPHABET.index(ch) - key[1]) % 26
                ]
...
class Playfair:
    @staticmethod
    def buildtable(key):
        return "".join(sorted(set(key), key=lambda x: key.index(x))) + "".join(
            [ch for ch in ALPHABET if not (ch in key) and ch != "J"]
        )

    # Padding message with X if two letters found in message in a row or
    # length of message is odd
    @staticmethod
    def padding(message):
    ...
    def substitution(message, table, *, mode):
        ...
        list_pos = [
            [
                [table.index(elem[0]) // 5, table.index(elem[0]) % 5],
                [table.index(elem[1]) // 5, table.index(elem[1]) % 5],
            ]
        ...
        for elem in list_pos:
            if elem[0][0] == elem[1][0]:
                list_pos2.append(
                    [
                        [elem[0][0], (elem[0][1] + mode) % 5],
                        [elem[1][0], (elem[1][1] + mode) % 5],
                    ]
                )
            elif elem[0][1] == elem[1][1]:
                list_pos2.append(
                    [
                        [(elem[0][0] + mode) % 5, elem[0][1]],
                        [(elem[1][0] + mode) % 5, elem[1][1]],
                    ]
                )
            else:
                list_pos2.append(
        ...
        c = "".join(
            [
                table[e[0][0] * 5 + e[0][1]] + table[e[1][0] * 5 + e[1][1]]
                for e in list_pos2
            ]
        )
        return c
...
if __name__ == "__main__":
    ...
    print("---Extended Vigenere---")
    test = "DEFENDTHEEASTWALLOFTHECASTLE"
    fin = readFileBinary("didik.jpg")
    c = ExtendedVigenere.encrypt(fin, "FORTIFICATION".upper())
    writeFileBinary("ciphertext", c)
    d = ExtendedVigenere.decrypt(c, "FORTIFICATION".upper())
    writeFileBinary("plaintext.jpg", d)
